...
public class ZSQBaseActivity extends Activity{
	...
	public void ExitQuest(){
		SharedPreferencesUtil spUtil = SharedPreferencesUtil.getInstance(ZSQBaseActivity.this);
		spUtil.saveEtime(PublicUtils.getEtime());
		String mobile = "";
		try {
			mobile = spUtil.getUserName();
		} catch (Exception e) {}
		new GetFlowStatisticsTask(ZSQBaseActivity.this).execute(MovieApplication.getAppVersionName(ZSQBaseActivity.this),  PublicUtils.getIMEI(ZSQBaseActivity.this), PublicUtils.getIMSI(ZSQBaseActivity.this),
	...
	class GetFlowStatisticsTask extends MovieAsyncTask<String, Integer, FlowStatisticsBean> {
		...
		//   super.onPreExecute();
		  ...
		  protected FlowStatisticsBean doInBackground(String... params) {
		   ...
		   MovieLib lib = MovieLib.getInstance(ZSQBaseActivity.this);
		   try {
		  ...
		  protected void onPostExecute(FlowStatisticsBean result) {
		   ...
		   if (exception != null) {
		    ToastAlone.showToast(ZSQBaseActivity.this, exception, Toast.LENGTH_SHORT);
		   }
		   if (result != null) {
			   System.out.println("----------GetFlowStatisticsTask tuichu result--------"+result);
