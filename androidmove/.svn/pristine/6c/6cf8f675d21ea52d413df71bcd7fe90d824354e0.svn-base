...
//@SuppressLint("NewApi")
public class ItotemWaterView extends AdapterView<Adapter> implements OnGestureListener {
	...
	// * 往上多移的距离
	// */
	// int bottom_excess_move = 0;
	//
	// /**
	// * 往下多移的距离
	// */
	// int top_excess_move = 0;
	...
	// * 顶部触发刷新距离
	// */
	// private final int MAX_TOP_SIZE = 100;
	// /**
	// * 底部触发刷新距离
	// */
	// private final int MAX_BOTTOM_SIZE = 100;
	...
	public ItotemWaterView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public ItotemWaterView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public ItotemWaterView(Context context) {
		super(context);
		init(context);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mAdapter != null) {
			...
			// 计算ViewGroup的可用区域
			int parentUsableWidth = parentWidth - getPaddingLeft() - getPaddingRight() - space_horizontal
					* (columns - 1);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (ev.getPointerCount() == 1) {
				...
				if (deltaY < 0) {
					// 下移
					// 判断上移 是否滑过头
					// if (bottom_excess_move == 0) {
					if (finalY > 0) {
						int move_this = Math.max(-finalY, deltaY);
						finalY = finalY + move_this;
						scrollBy(0, move_this);
					}
					// else if (finalY == 0) {// 如果已经是最顶端 继续往下拉
					// top_excess_move = top_excess_move - deltaY / 2;//
					// 记录下多往下拉的值
					// scrollBy(0, deltaY / 2);
					// }
					// }
				} else if (deltaY > 0) {
					// if (top_excess_move == 0) {
					...
					// else if (getMaxCloumnHeight() - finalY - heightPixels ==
					// 0) {
					// bottom_excess_move = bottom_excess_move + deltaY / 2;
					// scrollBy(0, deltaY / 2);
					// }
					// }
				}
		...
		case MotionEvent.ACTION_UP:
			// if (bottom_excess_move > 0) {
			// // 多滚了 要弹回去
			// scrollBy(0, -bottom_excess_move);
			// invalidate();
			// if (bottom_excess_move > MAX_BOTTOM_SIZE && onRefreshListener !=
			// null) {
			// onRefreshListener.onBottomRefresh();
			// }
			// bottom_excess_move = 0;
			// mTouchState = TOUCH_STATE_REST;
			//
			// }
			// if (top_excess_move > 0) {
			// // 多滚了 要弹回去
			// scrollBy(0, top_excess_move);
			// invalidate();
			// if (top_excess_move > MAX_TOP_SIZE && onRefreshListener != null)
			// {
...
//	}
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
		// if (bottom_excess_move == 0 && top_excess_move == 0) {
